...
| A moderator could moderate their own content | **Fixed 2026-08-20** (§7.16) — `requireNotOwnContent` inside `loadSignal` (plus `deleteComment` and `restoreSignal`, which load their own documents) refuses with `failed-precondition`. `addNote` is deliberately exempt. Needed a functions deploy |
