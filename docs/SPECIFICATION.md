...
**All of its text is selectable, and its links are live.** One `SelectionArea` wraps the
whole scroll body — one wrap for the screen rather than one per widget, so a drag can
start in the title and end inside a comment, and so `SelectableRegion` never has to
negotiate with `_timelineRow`'s `IntrinsicHeight`. Everything a person typed goes through
`LinkifiedText` (title, description, comment bodies, status and ownership notes, and
`SignalOwnerBlock`'s takeover note), which turns URLs, email addresses and phone numbers
into tappable spans over `link_parser.dart`. The parser is the security boundary and is
